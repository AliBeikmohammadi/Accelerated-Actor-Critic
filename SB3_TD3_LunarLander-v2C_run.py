...
###########################################################################################################################################################################################       
    ...
    #             action = np.array([f_y, f_angle])
                ...
                x, y, v_x, v_y = observations.cuda()[:,0], observations.cuda()[:,1], observations.cuda()[:,2], observations.cuda()[:,3]
                angle, v_angle, contact_left, contact_right = observations.cuda()[:,4], observations.cuda()[:,5], observations.cuda()[:,6], observations.cuda()[:,7]
                or_function = th.max(contact_left,contact_right)
                f_y= th.where(or_function > 0, -10. * v_y - 1., 5.5 * th.abs(x) - 10. * y - 10. * v_y - 1.)
                f_angle= th.where(or_function > 0, 0.*x , -th.clamp(5. * x + 10. * v_x, min=-4., max=4.) + 10. * angle + 20. * v_angle)
                hu_actions=th.stack((f_y, f_angle))
                hu_actions=th.t(hu_actions)
                #Noise type 1
...
def run_main(Seed= 3510913823, MSE_Weight=1, end_iteration_number=10, Noise_Prob=0, Noise_STD=0):
    SEED=Seed 
    algo= 'TD3'
    ...
    if SEED==None:
        np.random.seed(SEED)
    env = gym.make(envname) #without any wrapper and monitor
    ...
    model = TD3('MlpPolicy', env, learning_rate=0.001, buffer_size=200000, learning_starts=learning_starts, 
                batch_size=100, tau=0.005, gamma=0.98, train_freq=train_freq, gradient_steps=-1, 
                action_noise=NormalActionNoise(mean,sigma), replay_buffer_class=None, replay_buffer_kwargs=None, 
                optimize_memory_usage=False, policy_delay=2, target_policy_noise=0.2, 
                target_noise_clip=0.5, tensorboard_log=Save_results, create_eval_env=True, 
                policy_kwargs=dict(net_arch=[400, 300]), verbose=1, seed=SEED, device='auto', _init_setup_model=True,
