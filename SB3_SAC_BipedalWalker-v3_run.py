...
###########################################################################################################################################################################################
            ...
            w=th.tensor([[ 0.9, -0.7,  0.0, -1.4],
                        ...
                        [-2.7, -2.6,  0.3,  0.6],
                        [ 2.0,  2.8,  0.0, -0.9],
                        [-2.2,  0.6,  4.7, -4.6]], dtype=th.float, device=th.device("cuda"))
            b=th.tensor([3.2, 6.1, -4.0, 7.6], dtype=th.float, device=th.device("cuda"))
            hu_actions=th.add(th.mm(observations.cuda(), w), b) 
            #Noise Type 2
            hu_actions=th.add(hu_actions, self.sigma * th.randn(actions_pi.shape[1], device=th.device("cuda"))+ self.mu) 
            hu_actions=hu_actions.view(actions_pi.shape[0], actions_pi.shape[1])   
...
def run_main(Seed= 2778197655, MSE_Weight=1, end_iteration_number=10, SIGMA=0, MU=0):
    SEED=Seed
    algo= 'SAC'
    ...
    train_freq=64
    model = SAC('MlpPolicy', env, learning_rate=0.00073, buffer_size=300000, 
                learning_starts=learning_starts, batch_size=256, tau=0.02, gamma=0.98, 
                train_freq=train_freq, gradient_steps=64, action_noise=None, 
                ...
                use_sde=True, sde_sample_freq=- 1, use_sde_at_warmup=False, 
                tensorboard_log=Save_results, create_eval_env=False, policy_kwargs=dict(log_std_init=-3, net_arch=[400, 300]), 
                verbose=1, seed=SEED, device='auto', _init_setup_model=True,
                Adaptive_Beta=Adaptive_Beta, Just_Beta=Just_Beta, MSE_Weight=MSE_Weight, Initial_Beta=Initial_Beta,  start_iteration_number=start_iteration_number , end_iteration_number=end_iteration_number, SIGMA=SIGMA, MU=MU)
