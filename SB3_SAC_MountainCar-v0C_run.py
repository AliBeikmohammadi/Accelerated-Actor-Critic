...
#                     hu = np.array([force,])
            ...
            positions, velocities = observations.cuda()[:,0], observations.cuda()[:,1]
            cond1 = th.gt(positions, -4. * velocities) #https://jhui.github.io/2018/02/09/PyTorch-Basic-operations/
            cond2 = th.lt(positions, 13. * velocities - 0.6)
            or_function = th.max(cond1,cond2)
            hu_actions= 2.*or_function-1. #-1 and 1
            ...
            if self.initial_beta == 100: #just MSE loss
...
def run_main(Seed= 4258880664, MSE_Weight=1, end_iteration_number=10, Noise_Prob=0, Noise_STD=0):
    SEED=Seed
    algo= 'SAC'
    ...
    learning_starts=0
    train_freq=32 
    model = SAC('MlpPolicy', env, learning_rate=0.0003, buffer_size=50000, 
                 learning_starts=learning_starts, batch_size=512, tau=0.01, gamma=0.9999, 
                 train_freq=train_freq, gradient_steps=32, action_noise=None, 
                 replay_buffer_class=None, replay_buffer_kwargs=None, 
                 optimize_memory_usage=False, ent_coef=0.1, 
                 target_update_interval=1, target_entropy='auto', 
                 use_sde=True, sde_sample_freq=- 1, use_sde_at_warmup=False, 
                 tensorboard_log=Save_results, create_eval_env=False, policy_kwargs=dict(log_std_init=-3.67, net_arch=[64, 64]), 
