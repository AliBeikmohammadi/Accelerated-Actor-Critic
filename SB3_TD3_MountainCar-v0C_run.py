...
#                     hu = np.array([force,])
                ...
                cond1 = th.gt(positions, -4. * velocities) 
                cond2 = th.lt(positions, 13. * velocities - 0.6)
                or_function = th.max(cond1,cond2)
                hu_actions= 2.*or_function-1. #-1 and 1
                hu_actions=hu_actions.view(actions_pi.shape[0],actions_pi.shape[1]) #(100,1)
                #Noise type 1
...
def run_main(Seed= 2228253929, MSE_Weight=1, end_iteration_number=10, Noise_Prob=0, Noise_STD=0):
    SEED=Seed #2228253929
    algo= 'TD3'
    ...
    model = TD3('MlpPolicy', env, learning_rate=0.001, buffer_size=1000000, learning_starts=learning_starts, 
                ...
                action_noise=OrnsteinUhlenbeckActionNoise(mean, sigma, theta=0.15, dt=0.01, initial_noise=None), replay_buffer_class=None, replay_buffer_kwargs=None, 
