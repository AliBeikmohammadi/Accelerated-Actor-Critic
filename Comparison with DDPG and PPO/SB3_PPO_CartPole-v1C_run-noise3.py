...
def run_main(Seed= 1, algo= 'PPO', envname='CartPoleContinuous-v1', Adaptive_Beta=True, Just_Beta = True, MSE_Weight=1, Initial_Beta=1,  start_iteration_number=0 , end_iteration_number=10, SIGMA=0, MU=0):
    SEED=Seed #1
    ...
    env = make_vec_env('gym_examples/'+ envname, n_envs=8, seed=SEED, start_index=0, monitor_dir=None, wrapper_class=None, 
    ...
    env = VecNormalize(env, training=True, norm_obs=True, norm_reward=False, 
                      clip_obs=10.0, clip_reward=10.0, gamma=0.98, epsilon=1e-08, norm_obs_keys=None) #gamma should be same, norm_reward=False
    ...
    model = PPO('MlpPolicy', env, learning_rate=0.001, n_steps=n_steps, batch_size=256, n_epochs=20, gamma=0.98, gae_lambda=0.8, 
    ...
    total_iteration_number=np.ceil(total_timesteps/(env.num_envs * n_steps)) #PPO A2C
