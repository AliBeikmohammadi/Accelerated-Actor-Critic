...
arg_pass.add_argument(
  "--b",
  help='initial_beta; default 1  if 0: only R^T',
  type=float, 
  default=1,
)
arg_pass.add_argument(
...
Adaptive_Beta=args.Adap_B #[0]
Just_Beta = args.Just_B #[0]
MSE_Weight=args.w
Initial_Beta=args.b
...
def run_main(Seed= 1493355531, algo= 'PPO', envname='BipedalWalker-v3', Adaptive_Beta=True, Just_Beta = True, MSE_Weight=1, Initial_Beta=1,  start_iteration_number=0 , end_iteration_number=10, SIGMA=0, MU=0):
    SEED=Seed #1493355531
    ...
    env = make_vec_env(envname, n_envs=32, seed=SEED, start_index=0, monitor_dir=None, wrapper_class=None, 
    ...
    env = VecNormalize(env, training=True, norm_obs=True, norm_reward=False, 
                       clip_obs=10.0, clip_reward=10.0, gamma=0.999, epsilon=1e-08, norm_obs_keys=None) #gamma should be same, norm_reward=False

    new_logger = configure(Save_results+logname, ["stdout", "csv", "tensorboard"])
    n_steps=2048
    total_timesteps=5000000.0

    model = PPO('MlpPolicy', env, learning_rate=0.0003, n_steps=n_steps, batch_size=64, n_epochs=10, gamma=0.999, gae_lambda=0.95, 
                clip_range=0.18, clip_range_vf=None, normalize_advantage=True, 
